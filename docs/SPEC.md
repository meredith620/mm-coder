...
所有 IM 命令通过 `msg.isTopLevel`（是否为 channel 根帖）区分行为。回复目标规则：
- `isTopLevel=true`（channel 根帖）：回复到 channel（`threadId=''`），不创建 session
...
| 命令 | 行为 | 回复目标 |
|------|------|---------|
| `/open <name>` | 当前 thread 收确认 + 目标 session 绑定 thread 收定位消息 | 两个 target：当前 thread ack + 目标 binding.threadId |

#### C. 不支持的命令（thread 中返回错误，不落入 CLI）
...
| 命令 | 行为 | 回复目标 |
|------|------|---------|
| 其他 `/xxx`（非 A/B 类） | 回复"未知命令"错误，不创建 session | `''`（channel） |

#### E. 普通文本消息（仅 thread）

| 内容 | 行为 |
|------|------|
| 普通文本消息（非 `/` 开头） | 进入 Claude 处理流程，消息入队由 `IMMessageDispatcher` 串行处理 |
...
用户: /open bug-fix
Bot:  [在当前 thread 发确认，目标 session thread 发定位消息]
