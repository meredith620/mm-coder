...
### Mattermost / IM 连续执行语义

- [x] **MM 短 continuation prompt 语义增强**：对“继续 / 请继续执行 / 接着做”等短 continuation 文本，在进入 worker 前扩写为明确的继续执行提示，避免 Claude 只把它当作普通单轮回复
  - 对应 `src/daemon.ts`、`src/im-worker-manager.ts` 与 `tests/integration/message-delivery.test.ts`
