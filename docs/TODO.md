...
- [ ] **IM 审批改为 reaction 主路径 + `last` fallback**：主交互改为审批卡片 + reaction（👍 once / ✅ session / 👎 deny / ⏹️ cancel）；文本命令仅保留 `/approve last once|session`、`/deny last`、`/cancel last`
  - **依据**：完整 `requestId` 不应暴露为主交互对象；IM 审批应对齐 Claude Code TUI 的 once/session/deny/cancel 语义，并形成消息级审计闭环
